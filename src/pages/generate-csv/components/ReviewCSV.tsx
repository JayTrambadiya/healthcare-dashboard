...
if (
  ...
  !document.getElementById("skeleton-kf")
) {
...
const ReviewCSV: React.FC = observer(() => {
  const gridTheme = themeQuartz.withParams({
    headerBackgroundColor: "#f2f7f2",
    headerTextColor: "#1f3d27",
    headerColumnBorder: { color: "#d7e2d7" },
    rowBorder: { color: "#e4ebe4" },
    accentColor: "#2d6a6a",
  });
  ...
  const columnDefs: ColDef[] = useMemo(
    () =>
      csvStore.csvHeaders.map((header) => ({
        ...
        cellDataType: "auto",
        filter:
          inferCellDataType(header) === "number"
